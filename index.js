	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Olá @${num.split('@')[0]}🌍SEJA BEM VINDO🌍`
				client.sendMessage(mdata.id, teks, MessageType.text, {contextInfo: {"mentionedJid": [num]}})
			} else if (anu.action == 'remove') {
				num = anu.participants[0]
				teks = `Tchauzinho @${num.split('@')[0]}\n Um abraço do bot bahianinho😎👌`
				client.sendMessage(mdata.id, teks, MessageType.text, {contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			mess = {
				...
				only: {
					group: '[❗] Função somente para grupos, seu lindo! ❌',
					ownerG: '[❗] Função somente para grupos, seu lindo ❌',
					ownerB: '[❗] Perintah ini hanya bisa di gunakan oleh owner bot! ❌',
					admin: '[❗] AI NÃO NÉ MEMBRO COMUM! SOMENTE ADMS! ❌',
					Badmin: '[❗] O BOT PRECISA TER OS PODERES DE ADM PARA ESTE COMANDO, seu lindo.',
	       ...
	       const pushname = client.chats.get(mek.participant) === undefined ? (client.contacts[mek.key.remoteJid].notify ? client.contacts[mek.key.remoteJid].notify : "Kak") : (client.contacts[mek.participant].notify ? client.contacts[mek.participant].notify : "Kak")
			...
			case 'st':
				case 'sticker':
					...
					/*} else if ((isMedia || isQuotedImage) && colors.includes(args[0])) {
						...
						reply(`Digite ${prefix}sticker para fazer sua figurinha. sticker animado no maximo 8 seg.`)
					}
					break
				case 'tts':
			...
			case 'tagall':
				client.updatePresence(from, Presence.composing) 
					...
					mentions('╔══✪〘 mencionando geral 〙✪══\n╠➥'+teks+'╚═〘 BOT bahianinho 〙', members_id, true)
					break
                case 'tagall2':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
					...
					client.sendMessage(from, '╔══✪〘 Mention All 〙✪══\n╠➥'+teks+'╚═〘 BOT BAHIANINHO 〙', text, {quoted: mek})
					break
                case 'tagall3':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
